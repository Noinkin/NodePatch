...
export function applyPatch(modulePath: string, patchPath: string) {
    ...
    if (!fs.existsSync(resolvedModule))
        throw Logger.error(`Module not found: ${modulePath}`);
    if (!fs.existsSync(resolvedPatch))
        throw Logger.error(`Patch not found: ${patchPath}`);

    if (!require.cache[require.resolve(resolvedModule)])
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            if (
                ...
                fs.statSync(patchFile).isDirectory()
            ) {
