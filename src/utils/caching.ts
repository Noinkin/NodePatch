...
export class CodeVault {
    ...
    constructor(options?: CacheOptions) {
        this.baseDir =
            options?.baseDir || path.resolve(process.cwd(), ".vault");
        this.backend = options?.backend || "sqlite";
        this.dbPath = path.join(
            this.baseDir,
            options?.dbName || "codevault.db",
        );
    ...
    public store(key: string, code: string | Buffer): void {
        const rawData = Buffer.isBuffer(code)
            ? code
    ...
    public load(key: string): string | undefined {
        ...
        if (this.backend === "file") {
            ...
            const row = this.db
                .prepare(`SELECT code FROM code_store WHERE key = ?`)
                .get(key) as { code: Buffer };
